  ...
  let create ~num_features ~num_classes ~bias =
    ...
    let embed =
      if bias then
        ...
        let weight = Layer.embedding_weight embed in
        Tensor.(normal_ (narrow weight ~dim:1 ~start:0 ~length:(2 * num_features)) ~mean:1.0 ~std:0.02);
        Tensor.(zero_ (narrow weight ~dim:1 ~start:(2 * num_features) ~length:num_features));
        embed
      else
        let embed = Layer.embedding ~num_embeddings:num_classes ~embedding_dim:(2 * num_features) in
        Tensor.(normal_ (Layer.embedding_weight embed) ~mean:1.0 ~std:0.02);
        embed
    in
